...
//        annotator = new VariantAnnotator(refDict, chromosomeMap, new AnnotationBuilderOptions());
		...
		try {
			...
			BufferedWriter output = new BufferedWriter(new FileWriter(hgvs_file+"_Error.log"));
			...
			while ((strLine = br.readLine()) != null)  {
				...
				if(!hgvs.isCorrect()){
					output.write(strLine+"\t"+"Format Error\n"); 
					continue;
				}
				...
				if(parser.parse(hgvs)){
					...
					/**The HGVS ID will be searched by regex string */
						regexlist.add(hgvs);
						output.write(strLine+"\t"+"Not Found in DataBase\n"); 						
				}
   ...
   /** Output GenomicVariant translated from HGVS code in the VCF Format**/
	private void write2VCF(java.util.List<HGVS_Parser> variants,String vcf_file) throws AnnotationException {
		...
		header.addMetaDataLine(
				 new VCFInfoHeaderLine("CorrHGVS", VCFHeaderLineCount.UNBOUNDED, VCFHeaderLineType.Flag, "Corrected HGVS code, due to the original HGVS not in database"));
		...
		for(HGVS_Parser hgvs_parser:variants ){
			...
			if(!hgvs_parser.corrected){
				 vc = new VariantContextBuilder().chr(hgvs_parser.variant.getChrName()).
				  ...
				  vc = new VariantContextBuilder().chr(hgvs_parser.variant.getChrName()).
						...
						computeEndFromAlleles(alleles,hgvs_parser.variant.getPos()+1).
						attribute("ANN", annostr).
						attribute("HGVS",hgvs_parser.hgvs.getName()).attribute("CorrHGVS", 1).make();
